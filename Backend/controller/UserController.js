...
exports.resetPassword =catchAsyncErrors(async(req,res,next)=>{
  // create token hash
  const resetPasswordToken=crypto.createHash("ss#72").update(req.params.token).digest("hex");
  const user =await User.findOne({
    resetPasswordToken,
  ...
  if(req.body.password !== req.body.confirmPassword){
     return next(
       new ErrorHandler( " Reset password  is not matched with the password",400)
     );
  }
